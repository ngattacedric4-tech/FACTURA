...
import { Sparkles, X, Send, Shield, ChevronDown } from 'lucide-react';
...
export function AIAssistantAdmin() {
  ...
  useEffect(() => {
    if (open && messages.length === 0) {
      setMessages([{
        role: 'assistant',
        content: "Bonjour ! Je suis FACTURAI en mode administrateur.\n\nJ'ai accès aux données globales de la plateforme : entreprises inscrites, revenus, répartition des plans, activité récente. Posez-moi n'importe quelle question sur FACTURA.",
      }]);
    }
  ...
  const send = async (content: string = input.trim()) => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke('ai-chat', {
        body: { messages: newMessages, admin_mode: true },
      });
          ...
          >
            ...
            <div className="h-14 bg-[#111827] flex items-center px-4 gap-3 shrink-0">
              ...
              <div className="flex-1">
                <p className="text-white text-[13px] font-bold leading-tight">FACTURAI Admin</p>
                <p className="text-[11px] leading-tight" style={{ color: '#FCD34D' }}>
                  ● Mode administrateur · données globales
                </p>
              </div>
              ...
              {messages.map((m, i) => (
                <div key={i} className={`flex gap-2.5 ${m.role === 'user' ? 'flex-row-reverse' : ''}`}>
                  <div className={`w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5 ${
                    m.role === 'assistant' ? 'bg-[#111827]' : 'bg-amber-100'
                  }`}>
                    {m.role === 'assistant'
            ...
            <div className="p-3 border-t border-[#F3F4F6] shrink-0">
              <div className="flex gap-2">
                <input
                  ...
                  placeholder="Question sur la plateforme..."
